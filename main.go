...
func printUsage() {
	fmt.Printf("ev2md  (build: %s)  junsu.lee@servicenow.com\n", buildDate)
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  ev2md <input.enex> [-o output_dir] [-r]")
	fmt.Println("  ev2md <input.zip>  [-o output_dir] [-r]")
	...
	fmt.Println()
	fmt.Println("  # Convert all .enex/.zip files in a directory → written to ./output/")
